...
async def compress(event, msg, ffmpeg_cmd=0, ps_name=None):
    ...
    if ffmpeg_cmd == 1:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -c:v libx265 -preset fast -s 1280x720 -crf 25 -c:a libopus -ac 2 -ab 128k -c:s copy """{out}""" -y'
    elif ffmpeg_cmd == 2:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -c:v libx265 -preset fast -s 1920x1080 -crf 25 -c:a libopus -ac 2 -ab 128k -c:s copy """{out}""" -y'
    elif ffmpeg_cmd == 3:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -c:v libx265 -preset fast -s {wdt}x{hgt} -crf 25 -c:a libopus -ac 2 -ab 128k -c:s copy """{out}""" -y'
    elif ffmpeg_cmd == 4:
        cmd = f'ffmpeg -hide_banner -loglevel quiet -progress {progress} -i """{name}""" -c:v libx264 -preset fast -s {wdt}x{hgt} -crf 25 -c:a libopus -ac 2 -ab 128k -c:s copy """{out}""" -y'
    try:
